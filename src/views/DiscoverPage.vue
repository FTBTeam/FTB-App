<template>
  <div class="flex flex-1 flex-col min-h-full overflow-x-hidden">
    ...
    >
      ...
      >
        <div class="m-4 flex-1 relative flex mt-2 mb-3">
          ...
          <div
            class="absolute top-0 left-0 bg-transparent-black w-full z-10 frosted-glass"
            style="height: 3rem;"
            v-if="!swapping"
          >
            <p class="px-4 font-bold uppercase" style="line-height: 3rem;" v-if="getDisplayVideo(pack) !== null">
              {{ getDisplayVideo(pack).name }}
            </p>
          </div>
          <div class="absolute right-0 pl-2 flex flex-row z-10" style="bottom: 45px">
            <span
              v-for="(tag, i) in pack.tags.slice(0, 5)"
              :key="`tag-${i}`"
          ...
          <div :class="`flex-1 ${swapping ? 'hidden' : 'block'}`" style="z-index: 2;">
            <iframe
              v-if="index === 0 && getDisplayVideo(pack) !== null"
              width="100%"
              height="100%"
              @load="load"
              :src="iFrameURL(getDisplayVideo(pack).link)"
              frameborder="0"
    ...
    <div v-else class="carousel max-w-full flex flex-row flex-1">
      <div class="slide flex flex-col" style="min-width: 100%; padding: 0 20px; opacity:1;">
        <div class="m-4 flex-1 relative flex mt-2 mb-3">
          <div class="placeholder"></div>
...
.placeholder {
  background-image: url('../assets/images/ftb-no-pack-splash-normal.webp');
