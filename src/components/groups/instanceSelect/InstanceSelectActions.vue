...
import {faArrowRight, faCheckDouble, faCopy, faSquareMinus, faTimes, faTrash} from "@fortawesome/free-solid-svg-icons";
...
const allSelected = computed(() => {
  return instances.length === instanceStore.instances.length;
});
</script>
...
<template>
  <transition name="transition-fade">
    ...
    >
      <div
        class="bg-[#2a2a2a]/90 backdrop-blur border-1 border-white/15 shadow-lg rounded-lg py-3 px-5 flex items-center gap-6">
        <div class="text-white flex-1">
          <span class="px-2 py-1 bg-white/20 rounded mr-2 inline-block font-bold font-mono">{{
              instances.length
            }}</span> selected
        </div>
        ...
        <div class="flex gap-2">
          <div class="cursor-pointer p-1 rounded hover:bg-white/20"
               @click="allSelected ? emit('deselect-all') : emit('select-all')"
               :aria-label="`${allSelected ? 'Des' : 'S'}elect all instances`"
               data-balloon-pos="up"
          >
            <FontAwesomeIcon :icon="!allSelected ? faCheckDouble : faSquareMinus" fixed-width />
          </div>
  ...
  <MultiDuplicateModal @deselect="emit('deselect-all')" @close="multiDuplicateModal = false" :open="multiDuplicateModal"
                       :instances="instances"/>
  <MultiMoveModal @deselect="emit('deselect-all')" @close="multiMoveModal = false" :open="multiMoveModal"
                  :instances="instances"/>
</template>
