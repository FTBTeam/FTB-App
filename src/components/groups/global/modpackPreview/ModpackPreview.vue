<script lang="ts" setup>
  ...
  onMounted(async () => {
    ...
    loading.value = true;
    await nextTick(() => {
      modpackStore.getModpack(modpackId)
        .then(e => modpack.value = e)
...
<template>
  <div class="px-18 pt-20 absolute top-0 left-0 h-full bg-black/60 backdrop-blur z-50" :style="`width: ${ads.adsEnabled.value ? 'calc(100% - 400px)' : '100%'};`">
        ...
        <header class="p-5 relative z-[1] rounded-t-xl" :style="`background-image: url(${backgroundArtwork}); background-size: cover;`">
          ...
          <div class=" flex gap-6">
            ...
            <div>
              ...
              <p class="text-shadow mt-2">{{modpack.synopsis}}</p>
