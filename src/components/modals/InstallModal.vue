  ...
  <div class="flex flex-col min-w-1/2">
    ...
    <div class="flex mt-4 justify-center relative">
      <div class="relative w-1/2 block">
        <select
          class="rounded-none bg-input focus:outline-none focus:shadow-outline border border-input px-4 w-full h-full appearance-none leading-normal text-gray-400"
          v-model="version"
        >
          <option v-for="version in versions" :value="version.id" :key="`${packName}_${version.id}`"
            >{{ version.name }} - {{ version.type.toLowerCase() }}</option
          >
        </select>
