<template>
    <div class="flex flex-col my-2">
      <!-- <div class="flex flex-row justify-center my-4">
        <p class="text-white w-64 pr-4 mx-auto">{{label}}</p>
        <input class="bg-background focus:bg-background-lighten focus:outline-none border border-gray-700 block w-full p-2 appearance-none leading-normal text-gray-300" v-on:input="$emit('input', $event.target.value)" :value="value" v-on:blur="$emit('blur')"/>
