<template>
  <div class="flex flex-col min-w-1/2">
    <h1 class="text-4xl">
      {{ packName }}<small class="pl-2 text-base">{{ packVersion }}</small>
    </h1>
    <div>
      <ftb-slider
        label="Memory"
    ...
    <div class="mt-4 flex flex-row justify-center items-center">
      ...
      <ftb-button color="primary" class="mx-2" @click="saveSettings"
        ><span v-if="!loading">Save</span><font-awesome-icon icon="spinner" size="lg" spin v-else
      /></ftb-button>
    </div>
  </div>
</template>
...
@Component({
  ...
  name: 'SettingsModal',
  props: ['packName', 'packVersion', 'instance'],
})
