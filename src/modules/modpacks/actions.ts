...
import { InstallProgress, Instance, ModpackState } from './types';
...
export const actions: ActionTree<ModpackState, RootState> = {
  // async loadAllPacks({ commit, rootState, dispatch, state }) {
  ...
  async updateInstall({ commit, rootState }: any, install: InstallProgress) {
  ...
  refreshPacks({ dispatch, rootState }) {
  ...
  //   });
  // },
  // saveInstance({ dispatch, commit }, instance: Instance) {
  //   return new Promise((resolve, reject) => {
  //     dispatch(
  //       'sendMessage',
  //       {
  ...
  getChangelog({ commit, rootState }, { packID, versionID, type = 0 }) {
  ...
  fetchModpack({ commit, rootState }: any, packID) {
  ...
  fetchCursepack({ commit, rootState }: any, packID) {
    // logVerbose(rootState, 'Fetching modpack', packID);
    // if (rootState.modpacks.packsCache[packID]) {
    //   logVerbose(rootState, 'Found in cache', packID);
    //   return new Promise((resolve, reject) => resolve(rootState.modpacks.packsCache[packID]));
  ...
  async refreshCache({ commit, rootState, dispatch }: any) {
