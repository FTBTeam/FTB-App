...
//       const tags: ModPackTag[][] = await Promise.all(
...
//               return;
//             }
//             const packs: ModPack[] = await Promise.all(
//               packIDs.map(async (packID: number) => {
...
//       let packs: ModPack[] = await Promise.all(
//         allPacks.map(async packID => {
//           const pack: ModPack = await this.dispatch('modpacks/fetchModpack', packID, { root: true });
//           return pack;
//         }),
//       );
